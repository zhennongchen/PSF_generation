...
sheet = '/mnt/BPM_NAS/BPM/alldata/phase3_ge_origin/darwin/cleaned_labels/20240422_blur/all/Blur.csv'
sheet = pd.read_csv(sheet)
sheet['Blur'] = sheet['Blur'].fillna(0)
...
for index in range(0, sheet.shape[0]):
    ...
    save_folder_main = os.path.join('/mnt/BPM_NAS/BPM/alldata/phase3_ge_origin/darwin/simulations_v5_mgb_patches', patient_image_name)
    ...
    for random_i in range(0,3):
        ...
        # save info
        if patches == False:
            result.append([patient['Image'], random_i, ROI_use, PSFsize[0], limited_displacement * pixel_spacing[0], MaxTotalLength * pixel_spacing[0], patient['Blur'], patient['Exclude'], patient['Folder'], patient['MRN'], patient['SegFilename'], patient['Dataset']])
            df = pd.DataFrame(result, columns = ['Image', 'simulation', 'use_ROI?', 'PSFsize', 'limited_displacement(mm)', 'MaxTotalLength(mm)', 'Blur_in_original_image', 'Exclude', 'Folder', 'MRN', 'SegFilename', 'Dataset'])
        else:
            result.append([patient['Image'], random_i, ROI_use, PSFsize[0], limited_displacement * pixel_spacing[0], MaxTotalLength * pixel_spacing[0], patch_coordinate[0],patch_coordinate[1],patch_coordinate[2],patch_coordinate[3], patient['Blur'], patient['Exclude'], patient['Folder'], patient['MRN'], patient['SegFilename'], patient['Dataset']])
            df = pd.DataFrame(result, columns = ['Image', 'simulation', 'use_ROI?', 'PSFsize', 'limited_displacement(mm)', 'MaxTotalLength(mm)', 'patch_coordinate_x_min', 'patch_coordinate_x_max', 'patch_coordinate_y_min', 'patch_coordinate_y_max', 'Blur_in_original_image', 'Exclude', 'Folder', 'MRN', 'SegFilename', 'Dataset'])
...
#     save_folder = os.path.join('/mnt/BPM_NAS/BPM/alldata/phase3_ge_origin/darwin/real_blurs_mgb_patches', patient_image_name,'original_img')
...
#     if patches == False:
#         result.append([patient['Image'],  patient['Blur'], patient['Exclude'], patient['Folder'], patient['MRN'], patient['SegFilename'], patient['Dataset']])
#         df = pd.DataFrame(result, columns = ['Image', 'Blur_in_original_image', 'Exclude', 'Folder', 'MRN', 'SegFilename', 'Dataset'])
#     else:
#         result.append([patient['Image'], patch_coordinate[0],patch_coordinate[1],patch_coordinate[2],patch_coordinate[3], patient['Blur'], patient['Exclude'], patient['Folder'], patient['MRN'], patient['SegFilename'], patient['Dataset']])
#         df = pd.DataFrame(result, columns = ['Image', 'patch_coordinate_x_min', 'patch_coordinate_x_max', 'patch_coordinate_y_min', 'patch_coordinate_y_max', 'Blur_in_original_image', 'Exclude', 'Folder', 'MRN', 'SegFilename', 'Dataset'])
